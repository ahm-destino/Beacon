...
import { Auth, Onboarding, setToken } from '../../services/api';
...
export default function SignIn() {
  ...
  useEffect(() => {
    if (localStorage.getItem('beacon_session_expired')) {
      toast.error('Login session expired. Please sign in again.');
      localStorage.removeItem('beacon_session_expired');
    }
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorMsg = err?.error || err?.message || 'Login failed. Check your credentials.';
      
      // Show toast immediately - no redirect
  ...
  return (
    <AuthLayout>
      <h2 className="font-[var(--font-syne)] font-bold text-2xl text-[#0C4A6E] dark:text-[#F0F9FF] text-center mb-2">Welcome Back</h2>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-semibold text-[#0C4A6E] dark:text-[#F0F9FF]">Email or Phone</label>
          <input 
            type="text" 
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="text-right mt-1">
            <Link to="/auth/forgot-password" className="font-[var(--font-jakarta)] text-xs text-[#0284C7] dark:text-[#38BDF8] hover:underline font-medium">
              Forgot password?
            </Link>
          </div>
        </div>
      ...
      <p className="text-sm text-center text-[#0369A1] dark:text-[#7DD3FC] mt-6">
        ...
        <Link to="/auth/signup" className="font-semibold text-[#0284C7] dark:text-[#38BDF8] hover:underline">
          Get started free
        </Link>
      </p>
    </AuthLayout>
